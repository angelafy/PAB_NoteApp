import { Text, Box, Textarea, TextareaInput, FormControl, FormControlLabel, InputField, HStack, VStack } from 
"@gluestack-ui/themed"; import { Input as GlueInput } from "@gluestack-ui/themed"; import React from "react"; 
...
const Input = ({ dropdown, textarea, width, height, fontSize, label, value, secureTextEntry, keyboardType, onChangeText, disabled }) => { 
  ...
  return ( 
    <> 
      <FormControlLabel marginTop={"$5"} flexDirection="column" alignItems="left"> 
        <Text fontSize={fontSize ? fontSize : "$md"} fontWeight="bold" mb={"$2"}>           {label} 
        </Text> 
